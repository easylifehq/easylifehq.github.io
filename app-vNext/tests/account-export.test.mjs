...
test("whole-account export is versioned, deterministic, manifested, and secret-free", () => {
  ...
  assert.equal(parsed.schemaVersion, "easylife-account-export-v4");
  assert.deepEqual(parsed.manifest.compatibleWith, ["easylife-account-export-v1", "easylife-account-export-v2", "easylife-account-export-v3"]);
  assert.equal(parsed.metadata.timeZone, "America/Denver");
  assert.equal(parsed.metadata.weightUnit, "lb");
  assert.equal(parsed.manifest.included.length, 22);
