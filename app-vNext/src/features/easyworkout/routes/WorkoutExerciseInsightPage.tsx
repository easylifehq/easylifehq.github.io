...
export function WorkoutExerciseInsightPage() {
  ...
  if (!exercise) {
    return <PageSection headingLevel={1} eyebrow="Exercise detail" title="Not enough data yet" description="This exercise has no compatible saved history."><Link className="button-secondary" to="/app/easystatistics?tab=workout">Back to Workout progress</Link></PageSection>;
  }
    ...
    <PageSection headingLevel={1} eyebrow="Exercise detail" title={exercise.exerciseName} description={`${exercise.sessionCount} comparable sessions · ${exercise.trend} trend · ${exercise.trendConfidence} confidence.`}>
      ...
      <div className="statistics-app-grid">
        {exercise.records.map((record) => (
          <article className="statistics-insight-card" key={`${record.type}-${record.label}`}>
            ...
            <p>{record.previousValue === null ? "First compatible record" : `Previous best ${record.previousValue.toFixed(1)} ${record.unit}`} · <Link to={`/app/easyworkout/session/${encodeURIComponent(record.sourceWorkoutId)}`}>{record.performedOn}</Link></p>
          </article>
        ))}
      </div>
      <div className="workout-chart-alternative">
        ...
        <table><thead><tr><th>Date</th><th>Estimated 1RM</th><th>Source set</th><th>Confidence</th></tr></thead><tbody>
          {exercise.observations.map((point) => <tr key={point.sessionId}><td><Link to={`/app/easyworkout/session/${encodeURIComponent(point.sessionId)}`}>{point.performedOn}</Link></td><td>{point.estimatedOneRepMax.toFixed(1)} {settings.easyWorkout.weightUnit}</td><td>{point.sourceWeight} × {point.sourceReps}</td><td>{point.confidence}</td></tr>)}
        </tbody></table>
      </div>
      <Link className="button-secondary" to="/app/easystatistics?tab=workout">Back to Workout progress</Link>
    </PageSection>
  );
}
