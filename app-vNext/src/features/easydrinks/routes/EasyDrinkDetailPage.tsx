...
export function EasyDrinkDetailPage() {
  ...
  const { drinks, isLoading, error, saveDrink, copyDrink } = useEasyDrinks();
  ...
  const initialDraft: DrinkDraft = { name: drink.name, type: drink.type, ingredients: drink.ingredients, instructions: drink.instructions, notes: drink.notes, rating: drink.rating, tags: drink.tags, date: drink.date, favorite: drink.favorite, sourceDrinkId: drink.sourceDrinkId };
  return <PageSection eyebrow="Saved drink" title={drink.name} description="Review or edit the recipe, notes, rating, tags, date, and favorite status." headingLevel={1}><div className="toolbar-row"><button type="button" className="button-secondary compact-button" onClick={() => void copyDrink(drink).then((id) => navigate(withReviewMode(id ? `/app/easydrinks/${encodeURIComponent(id)}` : "/app/easydrinks", location.search)))}>Duplicate drink</button><Link className="ghost-button compact-button" to={withReviewMode("/app/easydrinks", location.search)}>Back to library</Link></div><DrinkEditorForm key={drink.updatedAt?.toISOString() || drink.id} initialDraft={initialDraft} submitLabel="Save changes" onSubmit={async (draft) => { await saveDrink(drink.id, draft); navigate(withReviewMode("/app/easydrinks", location.search)); }} onCancel={() => navigate(withReviewMode("/app/easydrinks", location.search))} /></PageSection>;
}
