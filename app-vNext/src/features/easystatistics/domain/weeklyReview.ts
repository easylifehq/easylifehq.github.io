...
export function deriveWeeklyReview(input: {
  ...
  const sections: WeeklyReviewSection[] = [
    ...
    { id: "workout", eyebrow: "Workout", title: recentWorkouts.length ? `${plural(recentWorkouts.length, "session")} in the last 7 days` : "No session in the last 7 days", summary: input.workoutGoalProgress || (recentWorkouts.length ? "Use the saved evidence to choose the next routine; recovery is not inferred." : "Start from a saved routine when training fits the week."), items: recentWorkouts.slice(0, 3).map((workout) => ({ id: workout.id, title: workout.routineName || "Workout", detail: workout.performedOn })), actionLabel: "Open Workout Progress", actionTo: "/app/easystatistics?tab=workout", needsAction: recentWorkouts.length === 0 },
  ];
