...
$uncheckedTasks = @(Select-String -Path "docs/codex/TASK_QUEUE.md" -Pattern "^\s*-\s+\[ \]" | ForEach-Object { $_.Line.Trim() })
$completedTasks = @(Select-String -Path "docs/codex/TASK_QUEUE.md" -Pattern "^\s*-\s+\[x\]" | ForEach-Object { $_.Line.Trim() })
...
if ($changedInLastCommit.Count -eq 0) {
    Write-Host "- None"
} else {
    $changedInLastCommit | ForEach-Object { Write-Host "- $_" }
}
Write-Host ""
...
if ($uncheckedTasks.Count -eq 0) {
    Write-Host "- None"
} else {
    $uncheckedTasks | ForEach-Object { Write-Host "- $_" }
}
Write-Host ""
...
Write-Host "## ChatGPT Pro Review Prompt"
Write-Host "Review this unattended Codex run as project lead. Decide whether to continue, revise the task queue, or stop. Return only small safe tasks suitable for docs/codex/TASK_QUEUE.md, with explicit forbidden scope."
