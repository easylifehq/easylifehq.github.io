...
Primary Action: Present the completed local P4.7-02 Today Review deduplication commit for EasyLife HQ review and a separate remote-backup decision.
Showable Moment: Today keeps one dominant Start here action while Review contains only distinct additional decisions before the unchanged Demo path.
...
Evidence Required: P4.7 requires clean preflight proof, baseline and final production builds, source-backed Today hierarchy and typed-identity evidence, desktop/mobile responsive checks, Capture focus/return regression proof, the integrated local/demo sequence, and final git diff checks.
Done Signal: P4.5, P4.6-01, and P4.7-01 are the accepted remote checkpoint at `afba7e79`; P4.7-02 is complete locally after excluding only the exact Start here identity from Review and passing desktop/mobile plus integrated local/demo verification.
...
Next Gate: EasyLife HQ review of the local P4.7-02 commit and a separate remote-backup decision. P5 remains paused.
...
Updated At: 2026-07-18 - P4.5, P4.6-01, and P4.7-01 are the accepted remote checkpoint at `afba7e79`. P4.7-02 gives Start here and Review typed presentation identities and removes only the exact Start here identity from Review, without changing selection, counts, destinations, fixtures, or stored data. Desktop and mobile browser checks, raw Capture focus/return, the fresh integrated local/demo route sequence, and the production build passed with no Firestore request, network failure, or console error during the successful smoke sequence. This is autonomous browser/source evidence, not human physical-device, physical-keyboard, screen-reader, authenticated production, or installed/PWA proof. P5 live AI and true push remain paused behind explicit approval.
...
Historical Stage 27 queue: docs/codex/NEXT_5_TASKS.md previously contained exactly five bounded private-alpha AI assistant tasks. It has now advanced to Stage 28 reliability tasks.
...
Completed Stage 26 queue: provider confirmation, server-shaped dry-run seam, validation/fallback proof, Inbox live dry-run source UI, and Stage 26 proof packet. `docs/codex/NEXT_5_TASKS.md` has been advanced to the Stage 27 queue.
