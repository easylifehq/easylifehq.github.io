...
- [ ] CAPTAIN PRIORITY authenticated visual QA repair: use the locally provided test login only during local dev inspection, do not store it in files, and inspect EasyList, EasyNotes, EasyCalendar, EasyWorkout, and Settings to identify why the app still looks unchanged; make one clearly visible UI-only protected-suite improvement in the shared shell, dashboard, or one protected feature screen. Do not change auth logic, Firebase config/rules, backend, persistence, data models, routing behavior, dependencies, package files, generated output, deployment config, secrets, docs report files, or root config files. [class:design risk:medium mode:single scope:app-vNext/src/ accept:npm.cmd run build]
- [ ] CAPTAIN PRIORITY Simon suite differentiation pass: make one protected EasyLife surface feel meaningfully more like a polished personal operating system instead of a generic form/card app by improving hierarchy, empty-state rhythm, or preview density using existing data and UI only; preserve behavior and do not touch auth, Firebase, backend, persistence, data models, routing, dependencies, package files, generated output, deployment config, secrets, docs report files, or root config files. [class:design risk:medium mode:single scope:app-vNext/src/ accept:npm.cmd run build]
